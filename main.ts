...
scene.onHitWall(SpriteKind.Player, function (sprite, location) {
    if (tiles.tileAtLocationEquals(tiles.getTileLocation(location.column, location.row), assets.tile`myTile`)) {
...
scene.onHitWall(SpriteKind.Shuriken, function (sprite, location) {
    if (tiles.tileAtLocationEquals(tiles.getTileLocation(location.column, location.row), sprites.castle.rock2)) {
        tiles.setWallAt(tiles.getTileLocation(location.column, location.row), false)
        tiles.setTileAt(tiles.getTileLocation(location.column, location.row), sprites.builtin.forestTiles10)
        music.play(music.melodyPlayable(music.knock), music.PlaybackMode.InBackground)
    }
})
function clearTouchesEnemies () {
    if (sprites.readDataBoolean(playerSprite, "touchesEnemy")) {
        enemiesTouched = 0
...
function spawnCoin (col: number, row: number) {
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Key, function (sprite, otherSprite) {
    ...
    sprites.setDataBoolean(sprite, "hasKey", true)
    sprite.startEffect(effects.spray, 1000)
    spawnEnemy(29, 12, false, true)
    spawnEnemy(27, 12, false, true)
    spawnEnemy(25, 12, false, true)
...
function spawnInitialEnemies () {
    spawnEnemy(10, 5, true, false)
    spawnEnemy(18, 12, true, false)
    spawnEnemy(22, 12, true, false)
    spawnEnemy(26, 12, true, false)
    spawnEnemy(25, 9, true, false)
    spawnEnemy(27, 9, true, false)
    spawnEnemy(29, 9, true, false)
    spawnEnemy(6, 12, true, false)
    spawnEnemy(8, 12, true, false)
    spawnEnemy(10, 12, true, false)
}
function isDangerGround (col: number, row: number) {
    dangerTileLocation = tiles.getTileLocation(col, row)
    return tiles.tileAtLocationEquals(dangerTileLocation, sprites.dungeon.hazardLava0) || (tiles.tileAtLocationEquals(dangerTileLocation, sprites.dungeon.hazardLava1) || tiles.tileAtLocationEquals(dangerTileLocation, sprites.swamp.swampTile13))
}
...
function spawnInitialItems () {
    ...
    tiles.placeOnTile(yellowNinjaSprite, tiles.getTileLocation(101, 10))
    spawnInitialCoins()
    spawnInitialFood()
...
function spawnEnemy (col: number, row: number, goLeft: boolean, isFast: boolean) {
    ...
    mySprite.setVelocity(getEnemySpeed(goLeft, isFast), 0)
    if (goLeft) {
        animation.runImageAnimation(
        mySprite,
        [img`
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (sprite, otherSprite) {
    if (!(sprites.readDataBoolean(playerSprite, "touchesEnemy"))) {
        ...
        numberOfJumps = 0
        Jump()
        sprites.setDataBoolean(playerSprite, "touchesEnemy", true)
